function check_inputs(sip_options::SIPOptions)
...
function core_sip_routine(lower_level_problem_1::Function, lower_level_problem_2::Function,
                          lower_bounding_problem::Function, upper_bounding_problem::Function,
                          set_xpbar::Function, ProblemStorage::SIPProblemStorage)

  # initializes solution
  UBDg = Inf; LBDg = -Inf; k = 0

  xbar, pbar, nx, np = set_xpbar(ProblemStorage)
  lower_disc_set = ProblemStorage.opts.lower_disc_set;
  upper_disc_set = ProblemStorage.opts.upper_disc_set
  ...
  # checks inputs
  eps_g, r = check_inputs(ProblemStorage.opts)

  # checks for convergence
  for k=1:ProblemStorage.opts.max_iterations

    # check for termination
    check_convergence(sip_result.lower_bound, sip_result.upper_bound, ProblemStorage.opts.tolerance) && (break)

    # solve lower bounding problem and check feasibility
    feas = lower_bounding_problem(lower_disc_set, sip_result, ProblemStorage)
    print_lbp!(ProblemStorage.opts, sip_result.lower_bound, sip_result.x_bar, feas)
    sip_result.feasibility = check_lbp_feasible(feas)

    # solve inner program  and update lower discretization set
    INNg1, feas = lower_level_problem_1(sip_result.x_bar,  sip_result, ProblemStorage)
    print_llp1!(ProblemStorage.opts, INNg1, sip_result.p_bar, feas)
    xstar, UBDg, return_flag = post_llp1_update(INNg1, ProblemStorage.opts.inner_tolerance,
    ...
    feas = upper_bounding_problem(upper_disc_set, eps_g, sip_result, ProblemStorage)
    print_ubp!(ProblemStorage.opts, sip_result.upper_bound, sip_result.x_bar, feas)
    if (feas)
      INNg2, feas = lower_level_problem_2(sip_result.x_bar,  sip_result, ProblemStorage)
      print_llp2!(ProblemStorage.opts, INNg2, sip_result.p_bar, feas)
      UBD_temp = copy(sip_result.upper_bound)
      sip_result.upper_bound, xstar, eps_g = post_llp2_update(INNg2, ProblemStorage.opts.inner_tolerance,
    ...
    print_int!(ProblemStorage.opts, k, sip_result.lower_bound, sip_result.upper_bound, eps_g, r)
