...
get_solution_time(x::NodeHistory) = get_lower_time(x) + get_upper_time(x) + get_lower_time(x) + get_upper_time(x)
...
get_relative_gap(x::NodeHistory) = abs.(get_upper_bound(x) - get_lower_bound(x))./min.(abs.(get_upper_bound(x)), abs.(get_lower_bound(x)))
